...
class PVE2_API extends Controller{
    protected $hostname;
    protected $username;
    protected $realm;
    protected $password;
    ...
    /*
     * bool login ()
     * Perform login to PVE Server using JSON API and obtain Access Ticket
     */
    public function login () {
        ...
        if ($login_ticket_data == null || $login_ticket_data['data'] == null) {
            ...
            // so we can identify when we need a new one expiration-wise later on
    ...
    public function setCookie() {
        if (!$this->check_login_ticket()) {
            throw new Exception("Not logged into Proxmox host. No valid Login access ticket found or ticket expired.", 3);
        }
     ...
     * Check if the login ticket is still valid, return FALSE if not.
     * At this moment, check is based on the age of ticket.
     */
    ...
    private function action ($action_path, $http_method, $put_post_parameters = null) {
        // Check if we have a prefix / before the path, if not add one
        ...
        // Decide what type of action we take
        switch ($http_method) {
            case "GET":
                // Nothing extra to do
                break;
     ...
     * Return the list of node names as provided by /api2/json/nodes.
     * We need this for future get/post/put/delete calls.
     * e.g. $this->get("nodes/XXX/status"); where XXX is one of the values retrieved from this returned array.
     */
     ...
     * Get Last VMID from Cluster or Node
     * return VMID or FALSE if not found.
     */
     ...
     * Return the version and minor revisions of Proxmox Server
     */
